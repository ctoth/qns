...
class ChordInputDriver:
    ...
    def start_reset(self, reset: str) -> None:
        ...
        bns = self._bns
        if self._tns:
            self._reset_scans = TNS_WARM_RESET_SCANS if reset == "warm" else TNS_COLD_RESET_SCANS
    ...
    def _advance_phase(self) -> None:
        ...
        if self._phase == "reset":
            if bns._reset_complete_writes > self._reset_complete_writes:
                if self._tns:
                    keyboard.queue_codes(tuple(code & 0x7F for code in reversed(self._reset_scans)))
    ...
    def _start_next_chord(self) -> None:
        ...
        boundary = bns._input_boundary
        assert boundary is not None
        if not self._has_consumed_input and bns._keyboard_ready_epoch <= max(
            self._ready_epoch, bns._keyboard_accept_epoch
        ):
            return
