...
pytestmark = pytest.mark.skipif(
    not CFFI_AVAILABLE,
    reason="run tools/build_ffi.py to enable legacy CFFI compatibility tests",
)
...
def test_asci_transmit_reaches_python_byte_callback(channel: int, value: int):
    ...
    program = bytes(
        (
            ...
            0xED,
            0x39,
            cntla_port,  # OUT0 (CNTLA),A
            0x3E,
            0x02,  # LD A,2: BSP's initial 9600-baud divisor
            0xED,
            0x39,
            cntlb_port,  # OUT0 (CNTLB),A
            0x3E,
            value,  # LD A,value
            0xED,
            0x39,
...
def test_asci_receive_interrupt_survives_disabled_interrupts(channel: int, value: int):
    ...
    program[:31] = bytes(
        (
            ...
            0xED,
            0x39,
            stat_port,  # OUT0 (STAT),A
            0x18,
            0xFE,  # JR $ while IFF1 is disabled
        )
    )
    ...
    assert received["rie_set_count"] == 1
...
def test_csio_exchange_crosses_native_callback_boundary(capfd):
    ...
    program = bytes(
        (
            ...
            0x10,  # AND TE
            0x20,
            0xF9,  # JR NZ back to the control read
            0x3E,
            0x20,  # LD A,RE
            0xED,
            0x39,
...
def test_io_callbacks_observe_exact_accumulated_cycle_positions():
    ...
    program = bytes(
        (
            0x3E,
            0x00,  # LD A,00h (6 cycles)
            0xED,
            0x39,
            0xA0,  # OUT0 (A0h),A (callback at cycle 6; 13 cycles)
            0x00,  # NOP (3 cycles)
            0x00,  # NOP (3 cycles)
            0x3E,
            0x20,  # LD A,20h (6 cycles)
            0xED,
            0x39,
            0xA0,  # OUT0 (A0h),A (callback at cycle 31)
            0x76,  # HALT
        )
    )
...
def test_csio_receive_raises_internal_interrupt():
    ...
    program[:23] = bytes(
        (
            ...
            0x33,  # OUT0 (IL),A
            0x3E,
            0x67,  # LD A,EIE|RE|external clock
            0xED,
            0x39,
            0x0A,  # OUT0 (CNTR),A
            0xFB,  # EI
